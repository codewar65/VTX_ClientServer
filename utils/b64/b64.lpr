...
uses
  ...
  cthreads,
  {$ENDIF}{$ENDIF}
  Base64, Classes, SysUtils, StrUtils;
...
begin
  ...
  // read input
	fin := TFileStream.Create(infname, fmShareCompat or fmOpenRead);
	len := fin.Size;
  buff := GetMemory(len);
  fin.ReadBuffer(buff[0], len);
  fin.Free;

  // get extention - lookup mime
  if domime then
	  begin
	  ext := ExtractFileExt(infname);
	  mime := '';
    for i := 0 to length(mimelut) - 1 do
	  begin
	    line := mimelut[i];
			vals := line.Split([',']);
      for j := 1 to length(vals) - 1 do
		    if LowerCase(vals[j]) = LowerCase(ext) then
		    begin
	  	  	mime := vals[0];
		  	  goto done;
		    end;
	  ...
	  begin
	    // unknown - determine
	    for i := 0 to len-1 do
	    	if not (buff[i] in [13,10,7,8,11,32..126]) then
	      begin
  ...
  rewrite(fout);
  if domime then
		write(fout, 'data:' + mime + ';base64,' + outstr)
  else
		write(fout, outstr);
  closefile(fout);
